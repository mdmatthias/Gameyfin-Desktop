...
class TestComboBoxGamepadNavigation:
    """Test gamepad interaction with QComboBox widgets."""

    @pytest.fixture
    def mock_combo_box(self):
        """Return a mock QComboBox for testing without Qt popup quirks."""
        cb = MagicMock()
        cb.count.return_value = 3
        cb.currentIndex.return_value = 1
        cb.currentText.return_value = "Script Two"
        return cb

    def test_open_combo_box_popup_calls_show_popup(self, mock_combo_box):
        """_open_combo_box_popup calls showPopup on the combo box."""
        from gameyfin_frontend.gameyfin_window import GameyfinWindow
        GameyfinWindow._open_combo_box_popup(MagicMock(), mock_combo_box)
        mock_combo_box.showPopup.assert_called_once()

    def test_select_combo_box_item_calls_launch_on_prefix_widget(self, mock_combo_box):
        """_select_combo_box_item delegates to PrefixItemWidget.launch_script."""
        from gameyfin_frontend.gameyfin_window import GameyfinWindow

        # Parent that has _navigating_popup and callable launch_script
        class FakeParent:
            _navigating_popup = True
            def __init__(self):
                self._launched_index = None
            def launch_script(self, index):
                self._launched_index = index

        parent = FakeParent()
        mock_combo_box.parent.return_value = parent

        window = MagicMock()
        GameyfinWindow._select_combo_box_item(window, mock_combo_box)

        assert parent._navigating_popup is False
        assert parent._launched_index == 1
        mock_combo_box.hidePopup.assert_not_called()

    def test_select_combo_box_item_fallback_for_plain_combos(self, mock_combo_box):
        """_select_combo_box_item falls back to hidePopup when no launch_script on parent."""
        from gameyfin_frontend.gameyfin_window import GameyfinWindow

        # Plain object without launch_script → getattr returns None → fallback
        class PlainParent:
            pass
        ...
        mock_combo_box.hidePopup.assert_called_once()
