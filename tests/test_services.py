...
class TestGameLauncher:
    ...
    def test_start_linux_builds_command(self, mock_settings):
        ...
        with patch("gameyfin_frontend.services.game_launcher.QProcess") as MockProcess:
            ...
            # Command no longer contains env vars — they're on the process environment
            command_str = call_args[0][1][1]
            assert "exec umu-run" in command_str
            assert '"/tmp/game/game.exe"' in command_str
            # Env vars are set on the process via setProcessEnvironment
            MockProcess.return_value.setProcessEnvironment.assert_called_once()

    @pytest.mark.skipif(sys.platform == "win32", reason="Linux-only test")
    def test_start_linux_includes_game_args(self, mock_settings):
        from gameyfin_frontend.services import GameLauncher
        ...
        with patch("gameyfin_frontend.services.game_launcher.QProcess") as MockProcess:
            ...
            result = launcher.start_linux(
                launcher_to_run="/tmp/game/game.exe",
                target_dir="/tmp/game",
                install_config={"USE_HOST_UMU": "1", "GAME_ARGS": "-windowed -memory=2048"},
                wine_prefix_path="/tmp/prefixes/my_game_pfx",
                proton_path="GE-Proton10",
            )
            ...
            call_args = MockProcess.return_value.start.call_args
            command_str = call_args[0][1][1]
            # Game args appear exactly as typed — no shell quoting
            assert 'umu-run "/tmp/game/game.exe" -windowed -memory=2048' in command_str

    @pytest.mark.skipif(sys.platform == "win32", reason="Linux-only test")
    def test_start_linux_game_args_with_quotes(self, mock_settings):
        """Quotes in game args appear exactly as typed — env vars are on the process, not the shell."""
        from gameyfin_frontend.services import GameLauncher
        ...
        with patch("gameyfin_frontend.services.game_launcher.QProcess") as MockProcess:
            ...
            result = launcher.start_linux(
                launcher_to_run="/tmp/game/launcher.exe",
                target_dir="/tmp/game",
                install_config={"GAME_ARGS": '--exec="launch WoWF"'},
                wine_prefix_path="/tmp/prefixes/wow_pfx",
            )

            assert result is not None
            call_args = MockProcess.return_value.start.call_args
            command_str = call_args[0][1][1]
            # Game args appear exactly as typed
...
class TestExeShortcutCreation:
    ...
    def test_per_script_config_gives_new_exe_the_games_settings(self, tmp_path):
        """With a per-script config.json (saved by Config), the new exe's script
        and its config entry get the game's protonfix and Proton path — not
        another script's game arguments."""
        from gameyfin_frontend.services.prefix_service import PrefixService
        ...
        with open(scripts_dir / "config.json", "w") as f:
            json.dump({
                "Game.sh": {
                    "PROTON_ENABLE_WAYLAND": "0", "MANGOHUD": "0", "PROTON_USE_WOW64": "0",
                    "GAMEID": "umu-367500", "STORE": "steam", "PROTONPATH": "Proton-Custom",
                    "EXTRA_VARS": "FOO=bar", "GAME_ARGS": "-game-only",
                },
            }, f)
        ...
        script = open(script_path).read()
        assert 'PROTONPATH="Proton-Custom"' in script
        assert 'GAMEID="umu-367500"' in script
        assert 'STORE="steam"' in script
        assert 'FOO="bar"' in script
        assert "-game-only" not in script
        assert "Game.sh" not in script

        with open(scripts_dir / "config.json") as f:
            saved = json.load(f)
        assert saved["tool.sh"]["GAMEID"] == "umu-367500"
        assert saved["tool.sh"]["PROTONPATH"] == "Proton-Custom"
        assert saved["tool.sh"]["GAME_ARGS"] == ""
        assert saved["Game.sh"]["GAME_ARGS"] == "-game-only"
    ...
    def test_recreated_scripts_keep_their_per_script_settings(self, tmp_path):
        """Manage > Shortcuts rebuilds scripts from a per-script config.json;
        each script must get its own env, not an empty one."""
        from gameyfin_frontend import utils

        prefix = tmp_path / "prefixes" / "my game_pfx"
        shortcuts_dir = prefix / "drive_c" / "proton_shortcuts"
        shortcuts_dir.mkdir(parents=True)
        for name in ("Game", "Tool"):
            (shortcuts_dir / f"{name}.desktop").write_text(
                f"[Desktop Entry]\nName={name}\nPath={prefix}\nStartupWMClass={name.lower()}.exe\n"
            )
        ...
        utils.create_shortcuts(
            all_desktop_files=[str(shortcuts_dir / "Game.desktop"), str(shortcuts_dir / "Tool.desktop")],
            scripts_dir=str(scripts_dir),
            wine_prefix=str(prefix),
            install_config={
                "Game.sh": {"GAMEID": "umu-1", "PROTONPATH": "P-Game", "EXTRA_VARS": "", "GAME_ARGS": "-g"},
                "Tool.sh": {"GAMEID": "umu-1", "PROTONPATH": "P-Tool", "EXTRA_VARS": "X=1", "GAME_ARGS": ""},
            },
        )

        game = (scripts_dir / "Game.sh").read_text()
        tool = (scripts_dir / "Tool.sh").read_text()
        assert 'PROTONPATH="P-Game"' in game and 'GAMEID="umu-1"' in game and 'game.exe" -g' in game
        assert 'PROTONPATH="P-Tool"' in tool and 'X="1"' in tool and tool.rstrip().endswith('tool.exe"')
        assert ".sh=" not in game + tool


class TestPrefixServiceUpdateScripts:
    def test_update_scripts_includes_game_args(self, tmp_path):
        """update_scripts appends GAME_ARGS from the config to the umu-run line."""
        from gameyfin_frontend.services.prefix_service import PrefixService
        ...
        # Write an existing .sh script (no game args yet)
        sh = scripts_dir / "game.sh"
        sh.write_text(
            '#!/bin/sh\n\n'
            'export LC_ALL=C.UTF-8\nexport LANG=C.UTF-8\n\n'
            'cd "/home/user/pfx/drive_c/Game"\n\n'
            '# Auto-generated by Gameyfin\n'
            'PROTONPATH="GE-Proton" WINEPREFIX="/home/user/pfx" umu-run "/home/user/pfx/drive_c/Game/game.exe"\n'
        )

        config = {
            "PROTONPATH": "GE-Proton",
            "GAME_ARGS": '--exec="launch WoWF"',
        }
        count = PrefixService(settings).update_scripts(
            prefix_path="/home/user/pfx",
            config=config,
            game_name="my game",
        )

        assert count == 1
        updated = open(sh).read()
        assert '--exec="launch WoWF"' in updated
        assert 'umu-run "/home/user/pfx/drive_c/Game/game.exe"' in updated
        # Should appear exactly once — not duplicated
        assert updated.count('--exec="launch WoWF"') == 1

    def test_update_scripts_replaces_existing_game_args(self, tmp_path):
        """Updating a script that already has game args replaces them, not duplicates."""
        from gameyfin_frontend.services.prefix_service import PrefixService
        ...
        # Script already has old game args
        sh = scripts_dir / "game.sh"
        sh.write_text(
            '#!/bin/sh\n\n'
            'export LC_ALL=C.UTF-8\nexport LANG=C.UTF-8\n\n'
            'cd "/home/user/pfx/drive_c/Game"\n\n'
            '# Auto-generated by Gameyfin\n'
            'PROTONPATH="GE-Proton" WINEPREFIX="/home/user/pfx" umu-run "/home/user/pfx/drive_c/Game/game.exe" --old-arg\n'
        )
        ...
        assert count == 1
        updated = open(sh).read()
        assert '--new-arg' in updated
        assert '--old-arg' not in updated
        assert updated.count('--new-arg') == 1

    def test_update_scripts_flat_per_script_format(self, tmp_path):
        """Per-script config from get_config() writes each script's own env vars,
        not the whole dict as a raw string."""
        from gameyfin_frontend.services.prefix_service import PrefixService
        ...
        # Write two existing .sh scripts
        sh1 = scripts_dir / "Battlenet.sh"
        sh1.write_text(
            '#!/bin/sh\n\n'
            'export LC_ALL=C.UTF-8\nexport LANG=C.UTF-8\n\n'
            'cd "/home/user/pfx/drive_c/Battle.net"\n\n'
            '# Auto-generated by Gameyfin\n'
            'umu-run "/home/user/pfx/drive_c/Battle.net/battle.net launcher.exe"\n'
        )
        sh2 = scripts_dir / "World of Warcraft Forever.sh"
        sh2.write_text(
            '#!/bin/sh\n\n'
            'export LC_ALL=C.UTF-8\nexport LANG=C.UTF-8\n\n'
            'cd "/home/user/pfx/drive_c/Warcraft"\n\n'
            '# Auto-generated by Gameyfin\n'
            'umu-run "/home/user/pfx/drive_c/Warcraft/Wow.exe"\n'
        )

        # Config in the exact format get_config() produces (no ALL_SCRIPTS key)
        config = {
            "Battlenet.sh": {
                "PROTON_ENABLE_WAYLAND": "1",
                "MANGOHUD": "0",
        ...
        # Battlenet.sh should have its own vars only
        content1 = sh1.read_text()
        assert 'BATTLE_VAR="hello"' in content1
        assert 'WOW_VAR="world"' not in content1
        assert 'Battlenet.sh=' not in content1  # no raw dict as env var
        assert 'umu-run "/home/user/pfx/drive_c/Battle.net/battle.net launcher.exe"' in content1

        # WoW script should have its own vars + game args
        content2 = sh2.read_text()
        assert 'WOW_VAR="world"' in content2
        assert 'BATTLE_VAR="hello"' not in content2
