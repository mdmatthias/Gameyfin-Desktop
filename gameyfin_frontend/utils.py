...
def shell_dquote(value: str) -> str:
    """Double-quote a value for embedding in a generated shell script.

    Unlike sanitize_name, this is for paths we can't rename (e.g. a game's
    install directory extracted from an archive, which may legitimately
    contain an apostrophe) — so it escapes for a double-quoted context
    instead of stripping characters.
    """
    for char in ('\\', '"', '$', '`'):
...
def create_shortcuts(
    ...
    for original_path in all_desktop_files:
        try:
            ...
            script_content = (
                f"#!/bin/sh\n\n"
                f"export LC_ALL=C.UTF-8\nexport LANG=C.UTF-8\n\n"
                f"cd {shell_dquote(working_dir)}\n\n"
                f"# Auto-generated by Gameyfin\n{command_to_run}\n"
            )
